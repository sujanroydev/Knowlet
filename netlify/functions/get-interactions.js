    ...
    try {
        ...
        if (action === 'likes') {
            ...
            if (user_id && page_id) {
                ...
                ({ data, error } = await supabaseClient
                    .from('interactions')
                    .select('page_ratings, ratings_message, interactions_time, users (name, picture)')
                    .eq('page_id', page_id)
                    .gt('page_ratings', 0)
                );
            }
            ...
            if (user_id && page_id) {
                ...
                ({ data, error } = await supabaseClient
                    .from('interactions')
                    .select('page_likes, page_ratings, ratings_message, interactions_time, users (name, picture)')
                    .eq('page_id', page_id)
                );
            }
        }
